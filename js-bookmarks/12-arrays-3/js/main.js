...
// Día 13:
const searchDescriptionInput = document.querySelector('.js_search_desc');
const searchSeenInput = document.querySelector('.js_search_seen');
const searchTagsAllInputs = document.querySelectorAll('.js_search_tags');
...
const GITHUB_USER = 'tuusuariodegithub';
...
let userSearchDescription = '';
let userSearchSeen = 'all';
let userSearchTags = [];
...
function renderBookmerksList() {
  // Vaciamos el innerHTML del <ul> de la página.
  listData.innerHTML = '';

  const filteredBmkData = bmkData
    .filter((bmk) => bmk.description.toLocaleLowerCase().includes(userSearchDescription)) // Día 13.1: Buscar por descripción
    .filter( // Día 13.2: Buscar por visto
      (bmk) =>
        userSearchSeen === 'all' ||
        (userSearchSeen === 'seen' && bmk.module) ||
        (userSearchSeen === 'unseen' && !bmk.module)
    )
    .filter((bmk) => userSearchTags.length === 0 || bmk.tags.some((tag) => userSearchTags.includes(tag))); // Día 13.3: Buscar por tags

  for (const oneBookmarkDataObject of filteredBmkData) {
    const newLiItem = renderBookmark(oneBookmarkDataObject);
    listData.appendChild(newLiItem);
  }
}

// Día 13.1: Buscar por descripción
searchDescriptionInput.addEventListener('keyup', (ev) => {
  userSearchDescription = searchDescriptionInput.value.toLocaleLowerCase();
  renderBookmerksList();
});

// Día 13.2: Buscar por leído
searchSeenInput.addEventListener('change', () => {
  userSearchSeen = searchSeenInput.value;
  renderBookmerksList();
});

// Día 13.3: Buscar por etiquetas
for (const searchInput of searchTagsAllInputs) {
  searchInput.addEventListener('change', (ev) => {
    if (ev.currentTarget.checked) {
      userSearchTags.push(ev.currentTarget.value);
    }
    else {
      const indexOfTagToRemove = userSearchTags.indexOf(ev.currentTarget.value);
      userSearchTags.splice(indexOfTagToRemove, 1);
    }
